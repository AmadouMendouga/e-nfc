...
public class AccountActivity extends Activity{
    ...
    // Android Beam (isNdefPushEnabled / setBeamPushUris) was removed from the SDK in Android 10
    // (API 29) and from every device running it. It is invoked through reflection here so the
    // app still compiles and runs against modern SDKs, matching the original behavior on the
    // old devices where Beam existed.
    public void sendFile(Money money){ // Deleted the view parameter from within the sendFile function
        ...
        if (Build.VERSION.SDK_INT >= 29){
            Toast.makeText(getApplicationContext(), "Android Beam was removed in Android 10+ and is not available on this device", Toast.LENGTH_LONG).show();
            return;
        }

        boolean beamEnabled;
        try {
            beamEnabled = (boolean) NfcAdapter.class.getMethod("isNdefPushEnabled").invoke(nfcAdapter);
        ...
        if (!beamEnabled){
            ...
            try {
                NfcAdapter.class.getMethod("setBeamPushUris", Uri[].class, Activity.class)
                        .invoke(nfcAdapter, (Object) new Uri[]{Uri.fromFile(fileToSend)}, this);
            } catch (ReflectiveOperationException e){
                e.printStackTrace();
            }
        }
